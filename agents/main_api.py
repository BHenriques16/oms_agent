...
def ask_demographics(payload: UserQuery):
    try:
        res_text, exec_time = run_with_metrics(
            DemographicsCrew(), payload.query, "Demographics"
        )
        return {
            "response": res_text,
...
def ask_incarcerations(payload: UserQuery):
    try:
        res_text, exec_time = run_with_metrics(
            IncarcerationsCrew(), payload.query, "Incarcerations"
        )
        return {
            "response": res_text,
            "execution_time_seconds": exec_time,
...
def ask_criminal_history(payload: UserQuery):
    try:
        res_text, exec_time = run_with_metrics(
            CriminalHistoryCrew(), payload.query, "Criminal History"
        )
        return {
            "response": res_text,
            "execution_time_seconds": exec_time,
...
def ask_supervision(payload: UserQuery):
    try:
        res_text, exec_time = run_with_metrics(
            SupervisionCrew(), payload.query, "Supervision"
        )
        return {
            "response": res_text,
            "execution_time_seconds": exec_time,
...
def ask_outcomes(payload: UserQuery):
    try:
        res_text, exec_time = run_with_metrics(
            OutcomesCrew(), payload.query, "Outcomes"
        )
        return {
            "response": res_text,
            "execution_time_seconds": exec_time,
...
def ask_count(payload: UserQuery):
    try:
        res_text, exec_time = run_with_metrics(
            CountCrew(), payload.query, "Count & Statistics"
        )
        return {
            "response": res_text,
            "execution_time_seconds": exec_time,
