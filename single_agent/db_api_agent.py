...
def search_inmates(
    ...
    try:
        with get_db_connection() as conn:
            ...
            valid_sort_columns = ['Age', 'RiskAssessmentScore', 'SentenceLengthMonths', 'CommissaryBalance', 'DisciplinaryIncidents']
            ...
            return {
                "filters_used": {
                    "block": block, "security_level": security_level, "min_risk": min_risk, 
                    "max_risk": max_risk, "min_age": min_age, "max_age": max_age,
                    "primary_offense": primary_offense, "is_parole_eligible": is_parole_eligible,
                    "gang_affiliation": gang_affiliation, "gender": gender, "ethnicity": ethnicity,
                    "health_status": health_status, "education_level": education_level,
                    "offense_keyword": offense_keyword, "behavioral_keyword": behavioral_keyword,
        ...
        raise HTTPException(status_code=500, detail="Database execution error during search.")
...
def get_full_profile(offender_id: int):
    try:
        with get_db_connection() as conn:
            ...
            if not row:
                raise HTTPException(status_code=404, detail=f"Inmate with ID {offender_id} not found in database.")
                
            return row_to_dict(cursor, row)
            
    except pyodbc.Error as db_err:
        print(f"[SQL Server Error in Profile]: {db_err}")
        raise HTTPException(status_code=500, detail="Internal server error while fetching inmate profile.")
...
def get_global_stats(
    ...
    try:
        with get_db_connection() as conn:
            ...
            if is_parole_eligible:
                if is_parole_eligible.lower() in ['true', '1']:
                    where_clause += " AND (IsParoleEligible = 1 OR IsParoleEligible = 'True')"
                else:
                    where_clause += " AND (IsParoleEligible = 0 OR IsParoleEligible = 'False')"
            if min_age is not None:
            ...
            if general_row and general_row[0] > 0:
                ...
                stats["general_metrics"] = {"TotalInmates": 0, "Message": "No inmates match the specified demographic filters."}
            
            # Fetch ONLY the top 10 most populated cell blocks
            if not block and general_row and general_row[0] > 0:
                cursor.execute(f"SELECT TOP 10 CellBlock, COUNT(*) as Qty FROM Offenders {where_clause} GROUP BY CellBlock ORDER BY Qty DESC", params)
                stats["top_10_populated_blocks"] = {row[0]: row[1] for row in cursor.fetchall()}
                
            if not security_level and general_row and general_row[0] > 0:
                cursor.execute(f"SELECT TOP 10 SecurityLevel, COUNT(*) as Qty FROM Offenders {where_clause} GROUP BY SecurityLevel ORDER BY Qty DESC", params)
                stats["by_security_level"] = {row[0]: row[1] for row in cursor.fetchall()}
            
            return stats
            
    except pyodbc.Error as db_err:
        print(f"[SQL Server Error in Stats]: {db_err}")
        raise HTTPException(status_code=500, detail="Internal server error while computing global statistics.")
